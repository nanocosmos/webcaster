...
3. **More Robust**: The new version is more robust than its predecessor. It offers improved error handling and stability, ensuring your live streams run smoothly. Because the standardized WHIP protocol is now used for negotiation of connections into the cloud, stream start issues caused by the proprietary websocket based protocol in version 5 will now be fixed.

4. **New Features**: v6 will introduce a range of new features that will enhance your live streaming capabilities. These features will be rolled out progressively, offering new possibilities for your application.
...
In version 6, we have switched to a promise-based model. Now, each client method returns a Promise. This Promise resolves when the operation is successful and rejects when an error occurs. This approach aligns with modern JavaScript practices and makes it easier to handle asynchronous operations. This also means that you can call any promise-returning method using the so-called async/await pattern that comes with modern JS runtimes. In this scenario, a try/catch clause can be used to handle errors. This is not a feature specific to this library, but a general language idiom coming with native runtime support nowadays.
...
Remember, you can also use async/await (as with any Promise-returning function), to avoid having to use closure scopes, or any callbacks at all. Clearly, both is functionally equivalent, and one or the other can be used interchangeably at any moment, depending on what fits better in your given context.
...
Here are the steps to migrate your application from nanoStream Webcaster v5 to v6:
